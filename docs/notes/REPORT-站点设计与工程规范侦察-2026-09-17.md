# REPORT — 站点设计与工程规范侦察（2026-09-17）

> 来源：董事长桌面稿 `C:\Users\a8881\Desktop\fengyuwang_com.md`（445 行 / 36KB，2026-09-17，dev 分支只读侦察）。
> 性质：**内部参考**。不对外发布、不进博客、不改任何站点页面 HTML。
> 处置（2026-09-18）：报告结论已按主题**并进**站内文稿，索引如下；此处保留原文（含原始侦察任务提示），供追溯。
>
> **并入索引**
> - §1 工程结构 / 发布根 → `README.md`「发布根与 Hugo 边界」；`docs/guide/release-gate.md` 门禁节速查 + `--article` 说明。
> - §2 设计语言（含 token 形式）→ `DESIGN.md` 头部「Token 的存在形式」；卡片栅格口径冲突修正于 `docs/guide/page-structure.md`。
> - §3 页面骨架 / h1-size / 导航注入 → `docs/guide/pitfalls.md` §14。
> - §4 多语与写作规范 → `docs/guide/voice-and-translation.md`「文章 front matter 与对齐门禁」；`docs/guide/page-structure.md` 新增页落点。
> - §5 资源与限制 / 可视化先例 / 现存缺陷 → `docs/guide/pitfalls.md` §14。
> - §6 已知雷区 / 落地清单 → `docs/guide/pitfalls.md` §14、`docs/guide/release-gate.md` 门禁节速查。
>
> **勘误（以代码实测为准，2026-09-18 复核）**
> - §1.1 / §6.1.13：`_redirects` 现有效规则 **29 条**（非原文「39」——39 是 2026-09-11 被覆盖前的历史行数，门禁基线为 ≥29）。
> - §3.4：手写页现为 **28 个/语言**（非 27），且已含 `system.html`（2026-09-18 新增，原文清单未列）；art 簇实际 **8 个**（非 9）。
> - §2.1：`--section-bg-img` 全仓出现 **456 处**（原文 126）；`assets/css/style.css` 0 个自定义属性、0 处 `var(--)` 的结论**正确**。
> - §5.1：Unsplash 引用为 **213 处/语言（三语共 639 处）**（原文「共 213」）。
> - §5.3：含 `fonts.googleapis.com` 的文件 **708 个**（原文 27，应为手写页数口径）。
> - §5.4：`system.html` 现已含手写 `<svg>`，「手写页 0 svg/canvas」不再是全站事实。
> - §2.3 卡片栅格：代码实测为 3 列（`repeat(3, minmax(0,1fr))`），原文「DESIGN.md 与 page-structure.md 口径不一致」属实，已按代码修正 page-structure.md。
> - 其余设计 token / 门禁函数名与阈值 / 现存 `human-in-the-loop-viewer.js` 缺失等结论经复核**属实**。
